...
public final class QueryUtils {
    ...
    public static List<LocationItem> fetchEarthquakeData(String requestUrl) {
        // Create URL object
        ...
        List<LocationItem> earthquake = extractEarthquakes(jsonResponse);
    ...
    /**
     * Method to create a URL from the URL String
     * @param stringUrl
     * @return
     */
    ...
    /**
         * Return a list of {@link LocationItem} objects that has been built up from
         * parsing a JSON response.
         */
    public static ArrayList<LocationItem> extractEarthquakes(String earthquakeJSON) {
        // If the JSON string is empty or null, then return early.
        if (TextUtils.isEmpty(earthquakeJSON)) {
            return null;
        }
        // Create an empty ArrayList that we can start adding earthquakes to
        ArrayList<LocationItem> earthquakes = new ArrayList<>();

        // Try to parse the SAMPLE_JSON_RESPONSE. If there's a problem with the way the JSON
        // is formatted, a JSONException exception object will be thrown.
        // Catch the exception so the app doesn't crash, and print the error message to the logs.
        try {
            JSONObject sampleJSONResponse = new JSONObject(earthquakeJSON);
            JSONArray features = sampleJSONResponse.getJSONArray("features");
            for(int i = 0; i<features.length(); i++)
            {
                ...
                earthquakes.add(new LocationItem(magnitude, place, time, url));
            }
